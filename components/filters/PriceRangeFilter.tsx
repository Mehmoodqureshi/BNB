...
const PriceRangeFilter: React.FC = () => {
  ...
  const priceOptions = [
    { value: 5000, label: 'Price Range' },
    { value: 500, label: 'Under AED 500' },
    { value: 1000, label: 'Under AED 1000' },
    { value: 2000, label: 'Under AED 2000' },
    { value: 5000, label: 'AED 2000+' }
  ];

  const selectedOption = priceOptions.find(option => option.value === filters.priceRange.max) || priceOptions[0];
            ...
            {priceOptions.map((option) => (
              <button
                ...
                className={`w-full px-4 py-3 text-left hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 ${
                  option.value === filters.priceRange.max
