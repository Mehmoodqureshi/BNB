...
const AdminHostsPage: React.FC = () => {
  ...
  const [selectedHost, setSelectedHost] = useState<UserManagementRecord | null>(null);
  const [showActionModal, setShowActionModal] = useState(false);

  // Mock hosts data
  const [hosts] = useState<UserManagementRecord[]>([
    {
      id: 'user_1',
      firstName: 'Sarah',
      lastName: 'Johnson',
    ...
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="flex-1 overflow-x-hidden">
        <AdminHeader 
          ...
          subtitle="Manage all platform hosts"
          actions={
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredHosts.map((host) => (
                    <tr key={host.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(host.status)}`}>
                          {host.status}
                        </span>
                        {host.flags.length > 0 && (
                          <div className="mt-1">
                            <span className="text-xs text-red-600 dark:text-red-400">
                              {host.flags.length} flag{host.flags.length > 1 ? 's' : ''}
                            </span>
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          <p className="text-gray-900 dark:text-white">{host.totalBookings} bookings</p>
                          <p className="text-gray-500 dark:text-gray-400">
                            {formatCurrency(host.totalEarnings || 0)}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-gray-900 dark:text-white">{formatDate(host.joinedDate)}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Active: {formatDate(host.lastActive)}
                        </p>
                      </td>
        ...
        {/* Action Modal */}
        {showActionModal && selectedHost && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md">
              <div className="p-6">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
                  Host Actions: {selectedHost.firstName} {selectedHost.lastName}
                </h3>
                <div className="space-y-2">
                  ...
                  {selectedHost.status === 'active' ? (
                    <button className="w-full flex items-center space-x-3 px-4 py-3 bg-yellow-50 dark:bg-yellow-900/20 hover:bg-yellow-100 dark:hover:bg-yellow-900/30 rounded-lg transition-colors text-left">
                      ...
                      <span className="text-yellow-900 dark:text-yellow-100">Suspend Host</span>
                    </button>
                  ) : (
                    <button className="w-full flex items-center space-x-3 px-4 py-3 bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/30 rounded-lg transition-colors text-left">
                      <CheckCircle className="h-4 w-4 text-green-600" />
                      <span className="text-green-900 dark:text-green-100">Reactivate Host</span>
                    </button>
                  )}
                  <button className="w-full flex items-center space-x-3 px-4 py-3 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors text-left">
                    ...
                    <span className="text-red-900 dark:text-red-100">Ban Host</span>
                  </button>
                </div>
